...
def ModImages(WeightsC2,B,R1,R2,PX,PY):
	...
	return Bx,Ux,Del2,SignB,minXB,STX,PulseB,PulseU


def ConvolutionSingle(WeightsC2,DelC,Bx,Ux,Del2,SignB,minXB,STX,PulseB,PulseU,Val,R1,R2,XC,YC,X1,X2,RX,RY):
	...
	for i in range(len(WeightsC2)):
		...
		DigitalVoltOutNew1[i]=TopConvolution2Mat64Latest(WeightsC2[i],Ux,PulseU,Div1,XC,YC,X1,X2,RX,RY,b);


	
	if(Val==1):
		if(STX==1):
			COut=[[sum([float((DigitalVoltOutNew[i][j][k]+SignB*(DigitalVoltOutNew1[i][j][k]))*DelC*Del2*0.5/a) for i in range(Split)]) for k in range(t2)] for j in range(t1)]
		if(STX==0):
			COut=[[sum([float((DigitalVoltOutNew[i][j][k]+SignB*minXB*(DigitalVoltOutNew1[i][j][k]))*DelC*Del2*0.5/a) for i in range(Split)]) for k in range(t2)] for j in range(t1)]
	if(Val==0):
		if(STX==1):
			COut=[[sum([float((DigitalVoltOutNew[i][j][k]+SignB*(DigitalVoltOutNew1[i][j][k]))*Del2*0.5/a) for i in range(Split)]) for k in range(t2)] for j in range(t1)]
		if(STX==0):
			COut=[[sum([float((DigitalVoltOutNew[i][j][k]+SignB*minXB*(DigitalVoltOutNew1[i][j][k]))*Del2*0.5/a) for i in range(Split)]) for k in range(t2)] for j in range(t1)]
	
	return COut


def TopConv(WeightC,DelC,Bx,Ux,DelB,SignB,minXB,STX,PulseB,PulseU,NegWeightsOut,DelC2,minC,PulseRes,RRAMRes,XC,YC,X1,X2,RX,RY):
	...
	for i in range(Images):
		PosWeightsOut[i]=ConvolutionSingle(WeightC[i],DelC[i],Bx[i],Ux[i],DelB[i],SignB[i],minXB[i],STX[i],PulseB[i],PulseU[i],1,PulseRes,RRAMRes,XC,YC,X1,X2,RX,RY)
...
def ConvolutionInMem(WeightC,DelC,WeightC2,DelC2,B,minC,PulseRes,RRAMRes,jobs):
	...
	for i in range(len(B)):
		Bxa,Uxa,DelBa,SignBa,minXBa,STXa,PulseBa,PulseUa,=ModImages(WeightC2,B[i],PulseRes,RRAMRes,PX,PY)
	...
	for i in range(len(B)):
		NegWeightsOut.append(ConvolutionSingle(WeightC2,DelC2,Bx[i],Ux[i],DelB[i],SignB[i],minXB[i],STX[i],PulseB[i],PulseU[i],0,PulseRes,RRAMRes,XC,YC,X1,X2,RX,RY))

	Outputs=[]
	for i in range(len(WeightC)):
		Outputs.append(TopConv(WeightC[i],DelC[i],Bx,Ux,DelB,SignB,minXB,STX,PulseB,PulseU,NegWeightsOut,DelC2[i],minC[i],PulseRes,RRAMRes,XC,YC,X1,X2,RX,RY))
	return Outputs
...
def ConvolutionComp8New(WeightX,ConvMatrixX,WeightX2,ConvMatrixX2,Div,X,Y,X1,X2,b):
	...
	#		Out1=(np.sum(Mod1,axis=0)+np.sum(Mod2,axis=0))*b
	#		Current1[bxe][cxe]=(currentI1*(N2/2)*M)-Out1
	return Current1
