...
AddF2 takes the binary inputs and maps it to 8x8 MH RRAM arrays. 
...
def ArrayFunction(N,A,B,BW,Var,TBP):
	l1=math.ceil(BW/N)
	Outs=N*4
	Ins=N*3
...
def ArrayProgramF(aB1,BW,Var):
	Arr=3
	Outs=math.pow(2,Arr)
	Nums=int(Outs/4)

	TBP=ArrayProgram(Nums,aB1,BW,Var)	
	return TBP	


def AddF(aB1,aB2,BW,Var,TBP):
	...
	Out=ArrayFunction(Nums,aB1,aB2,BW,Var,TBP)	
...
def Top(A,B,BW=32,Var=0):
	...
	PA=Division.BinaryCheck.CalMax(A,B)
	aB1,P1=Division.BinaryCheck.fractions(A,BW,PA)
	aB2,P2=Division.BinaryCheck.fractions(B,BW,PA)
