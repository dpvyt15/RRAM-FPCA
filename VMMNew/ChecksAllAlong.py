...
def ProgramMultiResSplitForJ5(A,R,Sp,Val,jobs):
	...
	if(Splits==2):
		for i in range(RowA):
			Ax[0][i]=[L[int(Mult*math.floor(A1t[i][j]/DelA))] for j in range(ColumnA)]
			Ay[0][i]=[L[int(Mult*math.ceil(A1t[i][j]/DelA))] for j in range(ColumnA)]
			Ax[1][i]=[L[int(Mult*math.floor(A2t[i][j]/DelA))] for j in range(ColumnA)]
			Ay[1][i]=[L[int(Mult*math.ceil(A2t[i][j]/DelA))] for j in range(ColumnA)]

	if(Splits==4):
		for i in range(RowA):
			Ax[0][i]=[L[int(Mult*math.floor(A11t[i][j]/DelA))] for j in range(ColumnA)]
			Ay[0][i]=[L[int(Mult*math.ceil(A11t[i][j]/DelA))] for j in range(ColumnA)] 
			Ax[1][i]=[L[int(Mult*math.floor(A12t[i][j]/DelA))] for j in range(ColumnA)] 
			Ay[1][i]=[L[int(Mult*math.ceil(A12t[i][j]/DelA))] for j in range(ColumnA)] 

			Ax[2][i]=[L[int(Mult*math.floor(A21t[i][j]/DelA))] for j in range(ColumnA)] 
			Ay[2][i]=[L[int(Mult*math.ceil(A21t[i][j]/DelA))] for j in range(ColumnA)] 
			Ax[3][i]=[L[int(Mult*math.floor(A22t[i][j]/DelA))] for j in range(ColumnA)] 
			Ay[3][i]=[L[int(Mult*math.ceil(A22t[i][j]/DelA))] for j in range(ColumnA)] 
    
	if(Splits==8):
		for i in range(RowA):
			Ax[0][i]=[L[int(Mult*math.floor(A111t[i][j]/DelA))] for j in range(ColumnA)]
			Ay[0][i]=[L[int(Mult*math.ceil(A111t[i][j]/DelA))] for j in range(ColumnA)]
			Ax[1][i]=[L[int(Mult*math.floor(A112t[i][j]/DelA))] for j in range(ColumnA)]
			Ay[1][i]=[L[int(Mult*math.ceil(A112t[i][j]/DelA))] for j in range(ColumnA)]

			Ax[2][i]=[L[int(Mult*math.floor(A121t[i][j]/DelA))] for j in range(ColumnA)]
			Ay[2][i]=[L[int(Mult*math.ceil(A121t[i][j]/DelA))] for j in range(ColumnA)]
			Ax[3][i]=[L[int(Mult*math.floor(A122t[i][j]/DelA))] for j in range(ColumnA)]
			Ay[3][i]=[L[int(Mult*math.ceil(A122t[i][j]/DelA))] for j in range(ColumnA)]

			Ax[4][i]=[L[int(Mult*math.floor(A211t[i][j]/DelA))] for j in range(ColumnA)]
			Ay[4][i]=[L[int(Mult*math.ceil(A211t[i][j]/DelA))] for j in range(ColumnA)]
			Ax[5][i]=[L[int(Mult*math.floor(A212t[i][j]/DelA))] for j in range(ColumnA)]
			Ay[5][i]=[L[int(Mult*math.ceil(A212t[i][j]/DelA))] for j in range(ColumnA)]

			Ax[6][i]=[L[int(Mult*math.floor(A221t[i][j]/DelA))] for j in range(ColumnA)]
			Ay[6][i]=[L[int(Mult*math.ceil(A221t[i][j]/DelA))] for j in range(ColumnA)]
			Ax[7][i]=[L[int(Mult*math.floor(A222t[i][j]/DelA))] for j in range(ColumnA)]
			Ay[7][i]=[L[int(Mult*math.ceil(A222t[i][j]/DelA))] for j in range(ColumnA)]
	...
	print(len(WeightsA))
	print(len(WeightsA[0]))
	print(len(WeightsA[0][0]))
...
def TopSetWeights(Ax,Ay,R):
	...
	A1x=[[L[int(Mult*Ax[i][j])] for j in range(ColumnA)] for i in range(RowA+1)]
	A1y=[[L[int(Mult*Ay[i][j])] for j in range(ColumnA)] for i in range(RowA+1)]

	AxX=VMMNew.GeneralFunctions.append_rows(A1x,A1y)
