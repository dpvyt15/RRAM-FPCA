...
def main():
	...
	for Epo in range(epochs):
		...
		for k in range(TrainBatches):
			...
			for start in range(minibatch*k,minibatch*(k+1)):
				...
				if ((max1-max2<0.25) or (Ax==0)):
					#print(start, TO[start], a1[len(a1)-1], next)
		...
		Accuracy=0
		print("Checking validation accuracy for {0} images for epoch {1}" .format(TotalTest))
	...
	Accuracy=0
	print("Checking test accuracy for {0} images" .format(TotalValid,Epo))
