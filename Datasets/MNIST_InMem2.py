...
"""5 layer CNN for MNIST database. In-memory convolution and VMM used for the classification. This is the top module that calls the different scripts for different tasks."""
...
def main():
	...
	for axe in range(len(cw)):
		WeightsW1,WeightsW2,DelW,DelW2,min=ConvolutionCheck.Programming.RRAMProgrammingMergeJ5(cw[axe],RRAMRes,Split,1,jobs);
		cW.append(WeightsW1)
	...
	for axe in range(len(lw)):
		WeightsW1,WeightsW2,DelW,DelW2,min=VMMNew.RRAM_Programming.RRAMProgrammingMergeJ5(lw[axe],RRAMRes,Split,0,jobs);
		lW.append(WeightsW1)
	...
	for Epo in range(epochs):
		...
		for k in range(TrainBatches):
			...
			for start in range(minibatch*k,minibatch*(k+1)):
				#print(start)
				XXX,x2,y1,o1,a1=FP_InMem2.TopInference(TI,start,cW,cW2,dcW,dcW2,mcW,cb,lW,lW2,dlW,dlW2,mlW,lb,poolsize,RRAMRes,PulseRes)
		...
		for ick in range(TotalValid):
			XXX,x2,y1,o1,a1=FP_InMem2.TopInference(VI,ick,cW,cW2,dcW,dcW2,mcW,cb,lW,lW2,dlW,dlW2,mlW,lb,poolsize,RRAMRes,PulseRes)
	...
	for i in range(TotalTest):
		XXX,x2,y1,o1,a1=FP_InMem2.TopInference(ti,i,cW,cW2,dcW,dcW2,mcW,cb,lW,lW2,dlW,dlW2,mlW,lb,poolsize,RRAMRes,PulseRes)
