...
def convolutionsTop(convw,convb,Input):

	OutNew=[[[[0 for i in range(3)] for j in range(3)] for k in range(len(Input))] for l in range(len(convw))]
	for i in range(len(Weight)):
		for j in range(len(Weight[0])):
			OutNew[i][j]=convolution(B[j],convw[i][j])
	...
	l5=int(len(convb))
	XXX=[[[OutNewX[k][i][j]+convb[k] for j in range(len(In[0][0]))] for i in range(len(In[0]))] for k in range(l5)]
	x2,y1=[],[]
	for i in range(l5):
		x2.append(maxpool(XXX[i]))
		
	y1=[[[max(i,0) for i in j] for j in k] for k in x2]
	return XXX,x2,y1

def VMM(Weight,In):
	return ([sum([Weight[i][j]*In[j] for j in range(len(In))]) for i in range(len(Weight))])

		
def TopInference(Input,start,cW,cW2,dcW,dcW2,mcW,convb,lW,lW2,dlW,dlW2,mlW,lb,RRAMRes,PulseRes):
	...
	for lm in range(len(cW)):
		X1,X2,Y1=ConvolutionCheck.ConvInMemP.ConvolutionInMem(cw[lm],cb[lm],y1[lm])
		XXX.append(X1)
		x2.append(X2)
		y1.append(Y1)
