...
def TopBackPropNew(a1,o1,lw,XXX,g1,y1,x2,convw,TrainIn,TrainOut,poolsize,gamma,beta,mean,var):
	...
	for lm in range(len(convw)-1,-1,-1):
		ReLuPOut=[]
		for i in range(l3):
			C2w,C2b,Mod[i]=ConvBackProp(g1[i][lm],x2[i][lm+1],XXX[i][lm],Mod[i],convw[lm],poolsize[lm])
			CwN[i].append(C2w)
			CbN[i].append(C2b)

			#print(len(Mod[i]),len(y1[i]),i,lm)
